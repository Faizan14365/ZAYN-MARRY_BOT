...
async function pairCommand(sock, chatId, message, q) {
    // Shared context info with ZAYN-Tech-Bot-V2 branding
    const newsletterMeta = {
        forwardingScore: 1,
        isForwarded: true,
        forwardedNewsletterMessageInfo: {
            newsletterJid: '120363405256521406@newsletter',
            newsletterName: '🛠️ZAYN-MARRY_BOT',
            serverMessageId: -1
        }
    };
    ...
    try {
        ...
        if (numbers.length === 0) {
            return await sock.sendMessage(chatId, {
                text: "❌ Invalid number format. Please use something like `9232143428375`.",
                contextInfo: newsletterMeta
            });
        }
